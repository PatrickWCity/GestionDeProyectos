...
{
    public partial class ConsultarRangoMontosTipoFuenteFinanciamiento : Form
    {
        TipoFuenteFinanciamiento p = null;
        ControladorTipoFuenteFinanciamiento cp = null;
        int swbn, monto1, monto2 = 0;
        public ConsultarRangoMontosTipoFuenteFinanciamiento()
        {
            InitializeComponent();
            Icon = Properties.Resources.Icon;
            l_ZonaMensaje.Text = "";

            swbn = (int)Properties.Settings.Default["TipoFuenteFinanciamiento"];
            if (swbn == 1)
            {
            ...
            cp = new ControladorTipoFuenteFinanciamiento(p);
            DataTable dt = new DataTable();
            DataTable dt1 = new DataTable();
            dt = cp.ConsultarPorTodos();

            Dictionary<int, string> test = new Dictionary<int, string>();
            test.Add(0, "Selecionar Tipo de Fuente de Financiamiento");
        ...
        private void b_BN_Click(object sender, EventArgs e)
        {
            ...
            Properties.Settings.Default["TipoFuenteFinanciamiento"] = swbn;
                ...
                if (int.TryParse(tb_monto1.Text, out monto1))
                {
                    ...
                    else
                    {
                        if (int.TryParse(tb_monto2.Text, out monto2))
                        {
                            if (monto1 < monto2)
                            {
                                p = new TipoFuenteFinanciamiento();
                                p.monto1 = monto1;
                                p.monto2 = monto2;
                                p.id_tff = ((KeyValuePair<int, string>)comboBox1.SelectedItem).Key;
                                cp = new ControladorTipoFuenteFinanciamiento(p);
                                DataTable dt = new DataTable();
